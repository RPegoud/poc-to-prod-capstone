...
class BaseTextCategorizationDataset:
    ...
    def get_label_to_index_map(self):
        ...
        """
        return {label:idx for idx, label in self.get_index_to_label_map().items()}

    def to_indexes(self, labels: list):
        """
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        # TODO: CODE HERE
        assert list(dataset.columns) == ['post_id', 'tag_name', 'tag_id', 'tag_position', 'title']
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                return df.groupby('tag_name').filter(lambda n: len(n) >= x)
        ...
        # TODO: CODE HERE
        return dataset.pipe(func=filter_tag_position(0))\
            .pipe(func=filter_tags_with_less_than_x_samples(min_samples_per_label))
