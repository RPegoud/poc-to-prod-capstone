...
class TextPredictionModel:
    ...
    def predict(self, text_list, top_k=5):
        ...
        y_preds = self.model.predict(embedded)

        # TODO: CODE HERE
        # from tags indexes compute top_k tags for each text
        indexes = argsort(y_preds[-top_k:]) # top_k indexes with the best score
        preds = [self.labels_index_inv[index] for index in indexes]
